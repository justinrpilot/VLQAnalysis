...
#include "TGraph.h"
//tZ = 1, bW = 2, tH = 3
void makeThetaTemplatesBB_genUp(int numTop, int numW, int numZ, int numH, int numB, float br_bw = 0.3333333, float br_tz = 0.3333333, float br_th = 0.333333, string extraWeight = "1", string label = "blah"){


cout << "Running " << numTop << " " << numW << " " << numZ << " " << numH << " " << numB << endl;

cout << "Extra weight = " << extraWeight.c_str() << endl;
...
TFile *fileHbb = TFile::Open("root://cmseos.fnal.gov//store/user/pilot/VLQAnaMay/outplots_Hbb.root", "READ");
TFile *fileHWW = TFile::Open("root://cmseos.fnal.gov//store/user/pilot/VLQAnaMay/outplots_HWW.root", "READ");
...
TFile *sig700 = TFile::Open("root://cmseos.fnal.gov//store/user/pilot/VLQAnaMay/outplots_vlqBB700.root", "READ");
TFile *sig800 = TFile::Open("root://cmseos.fnal.gov//store/user/pilot/VLQAnaMay/outplots_vlqBB800.root", "READ");
TFile *sig900 = TFile::Open("root://cmseos.fnal.gov//store/user/pilot/VLQAnaMay/outplots_vlqBB900.root", "READ");
TFile *sig1000 = TFile::Open("root://cmseos.fnal.gov//store/user/pilot/VLQAnaMay/outplots_vlqBB1000.root", "READ");
TFile *sig1100 = TFile::Open("root://cmseos.fnal.gov//store/user/pilot/VLQAnaMay/outplots_vlqBB1100.root", "READ");
TFile *sig1200 = TFile::Open("root://cmseos.fnal.gov//store/user/pilot/VLQAnaMay/outplots_vlqBB1200.root", "READ");
TFile *sig1300 = TFile::Open("root://cmseos.fnal.gov//store/user/pilot/VLQAnaMay/outplots_vlqBB1300.root", "READ");
TFile *sig1400 = TFile::Open("root://cmseos.fnal.gov//store/user/pilot/VLQAnaMay/outplots_vlqBB1400.root", "READ");
TFile *sig1500 = TFile::Open("root://cmseos.fnal.gov//store/user/pilot/VLQAnaMay/outplots_vlqBB1500.root", "READ");
TFile *sig1600 = TFile::Open("root://cmseos.fnal.gov//store/user/pilot/VLQAnaMay/outplots_vlqBB1600.root", "READ");
TFile *sig1700 = TFile::Open("root://cmseos.fnal.gov//store/user/pilot/VLQAnaMay/outplots_vlqBB1700.root", "READ");
TFile *sig1800 = TFile::Open("root://cmseos.fnal.gov//store/user/pilot/VLQAnaMay/outplots_vlqBB1800.root", "READ");
...
string var = "ht";
TString selT = Form("(%s) * (numJets == 4 && jetPt_j3 > 400. && numTop == %d && numW == %d && numZ == %d && numH == %d && numB == %d)", extraWeight.c_str(), numTop, numW, numZ, numH, numB);
...
//selT = Form("(numJets == 4 && jetPt_j3 > 400. && numTop == %d && numW == %d && numZ == %d && numH == %d && numB == %d)", numTop, numW, numZ, numH, numB);
//TString selT = Form("numJets == 3");
//sel = selT.Data();
...
TString sigselT = Form("(%s) * (((decayMode1 == 1 && decayMode2 == 1) || (decayMode2 == 1 && decayMode1 == 1))*%f + ((decayMode1 == 1 && decayMode2 == 2) || (decayMode1 == 2 && decayMode2 == 1))*%f + ((decayMode1 == 1 && decayMode2 == 3) || (decayMode1 == 3 && decayMode2 == 1))*%f + ((decayMode1 == 2 && decayMode2 == 3) || (decayMode1 == 3 && decayMode2 == 2))*%f +((decayMode1 == 2 && decayMode2 == 2) || (decayMode2 == 2 && decayMode1 == 2))*%f + ((decayMode1 == 3 && decayMode2 == 3) || (decayMode2 == 3 && decayMode1 == 3))*%f  ) * (numJets == 4 && jetPt_j3 > 400. && numTop == %d && numW == %d && numZ == %d && numH == %d && numB == %d)", extraWeight.c_str(), 3*br_tz*3*br_tz, 3*br_tz*3*br_bw, 3*br_tz*3*br_th, 3*br_bw*3*br_th, 3*br_bw*3*br_bw, 3*br_th*3*br_th, numTop, numW, numZ, numH, numB);
...
//TFile *sampledF = TFile::Open(Form("shapes_%dt_%dW_%dZ_%dH_%db.root",numTop, numW, numZ, numH, numB), "READ");

gROOT->cd();
TH1F * histQCD_sampled = (TH1F *) sampledF->Get("outHT")->Clone("histQCD_sampled");
TH1F * histQCD_sampled_up = (TH1F *) sampledF->Get("outHTup")->Clone("histQCD_sampled_up");
TH1F * histQCD_sampled_dn = (TH1F *) sampledF->Get("outHTdn")->Clone("histQCD_sampled_dn");
...
for (int i = 0; i < histQCD_sampled->GetNbinsX(); i++){
	totalH->SetBinError(i, histQCD_sampled_up->GetBinContent(i) - histQCD_sampled->GetBinContent(i) );
}
...
TLegend *leg = new TLegend(0.7,0.15,0.92,0.86,"");
leg->SetNColumns(1);
leg->SetFillColor(0);
leg->SetLineColor(0);
...
cout << "HERE " << endl;
TFile *outFile = new TFile(TString::Format("%s_histos_%sUp.root", channel.c_str(), label.c_str()), "RECREATE");

histTT->Write(TString::Format("%s__ttbar__%s__plus", channel.c_str(), label.c_str()));
histWj->Write(TString::Format("%s__wjets__%s__plus", channel.c_str(), label.c_str()));
histZj->Write(TString::Format("%s__zjets__%s__plus", channel.c_str(), label.c_str()));
histWW->Write(TString::Format("%s__diboson__%s__plus", channel.c_str(), label.c_str()));
histTTW->Write(TString::Format("%s__ttV__%s__plus", channel.c_str(), label.c_str()));
histHbb->Write(TString::Format("%s__higgs__%s__plus", channel.c_str(), label.c_str()));

hist_sig700->Write(TString::Format("%s__tprime700__%s__plus", channel.c_str(), label.c_str()));
hist_sig800->Write(TString::Format("%s__tprime800__%s__plus", channel.c_str(), label.c_str()));
hist_sig900->Write(TString::Format("%s__tprime900__%s__plus", channel.c_str(), label.c_str()));
hist_sig1000->Write(TString::Format("%s__tprime1000__%s__plus", channel.c_str(), label.c_str()));
hist_sig1100->Write(TString::Format("%s__tprime1100__%s__plus", channel.c_str(), label.c_str()));
hist_sig1200->Write(TString::Format("%s__tprime1200__%s__plus", channel.c_str(), label.c_str()));
hist_sig1300->Write(TString::Format("%s__tprime1300__%s__plus", channel.c_str(), label.c_str()));
hist_sig1400->Write(TString::Format("%s__tprime1400__%s__plus", channel.c_str(), label.c_str()));
hist_sig1500->Write(TString::Format("%s__tprime1500__%s__plus", channel.c_str(), label.c_str()));
hist_sig1600->Write(TString::Format("%s__tprime1600__%s__plus", channel.c_str(), label.c_str()));
hist_sig1700->Write(TString::Format("%s__tprime1700__%s__plus", channel.c_str(), label.c_str()));
hist_sig1800->Write(TString::Format("%s__tprime1800__%s__plus", channel.c_str(), label.c_str()));
