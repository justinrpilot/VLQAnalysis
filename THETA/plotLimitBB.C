...
double max = 200000.0; //band_exp2->GetHistogram()->GetMaximum()*50;
  ...
  limit_exp->Draw("AL");
  limit_exp->GetXaxis()->SetTitle("M_{B} [GeV]");
  limit_exp->GetYaxis()->SetTitle("95% CL Limit on #sigma / #sigma(BB)");
  ...
  l->AddEntry(theory, "BB Expectation", "L");
  l->AddEntry(theory, Form("BR(bZ, bH, tW) = "), "");
  l->AddEntry(theory, Form("%1.1f, %1.1f, %1.1f", BRTZ, BRTH, BRBW), "");
  //l->AddEntry(theory, Form("BR(bZ) = %1.1f", BRTZ), "");
  //l->AddEntry(theory, Form("BR(bH) = %1.1f", BRTH), "");
  //l->AddEntry(theory, Form("BR(tW) = %1.1f", BRBW), "");
  l->SetFillColor(0);
  l->SetLineColor(0);
  l->Draw();
...
CMS_lumi(canvas, 4, 10);
  ...
  canvas->Print(Form("limitBB_%1.1ftz_%1.1fbw_%1.1fth.pdf", BRTZ, BRBW, BRTH));
