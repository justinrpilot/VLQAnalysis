...
def allhadfilter(hname):
	...
	keep = False
    if 'tprime' in process and '1200' not in hname:
	keep = False
    if '0t_0W_0Z_0H_0b' not in hname:
	keep = False
...
expected, observed = bayesian_limits(model, 'all', n_toy = 80, run_theta = True)
expected.write_txt("limitsTEST.txt")
observed.write_txt("obslimitsTEST.txt")
