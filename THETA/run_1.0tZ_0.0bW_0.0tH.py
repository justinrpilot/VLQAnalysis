...
infile = "templates/theta4jet_1.0tz_0.0bw_0.0th.root" 
...
scan_dict = nll_scan(model, input='data', n=1, range=[0.0,300.0], options = options)
for sig in scan_dict:
	print scan_dict[sig][0]
#expected, observed = bayesian_limits(model, 'all', n_toy = 5, run_theta = True)
#expected.write_txt("limitsJAN_1.0tz_0.0bw_0.0th.txt")
#observed.write_txt("obslimitsJAN_1.0tz_0.0bw_0.0th.txt")
...
options.set('minimizer', 'strategy', 'robust')
options.set('minimizer', 'minuit_tolerance_factor', '10000000')
parVals = mle(model, input='toys-asimov:0', n=1, options = options)
#print parVals
#with open('pulls_APPROVAL_ASIMOV.json', 'w') as file:
#    file.write(json.dumps(parVals))
#    parameter_values = {} 
#    parameter_values_nom = {} 
#    for p in model.get_parameters([]):
