...
TFile *rates = new TFile("BEST_rates_DATA_6bin.root", "READ");
...
TH1D *outHT = new TH1D("outHT", "outHT", nbins, bins);
TH1D *outHTup = new TH1D("outHTup", "outHTup", nbins, bins);
TH1D *outHTdn = new TH1D("outHTdn", "outHTdn", nbins, bins);
...
*/
	...
	outHT->Fill(ht, jet0wt*jet1wt*jet2wt);
	outHTup->Fill(ht, jet0wtup*jet1wtup*jet2wtup);
	outHTdn->Fill(ht, jet0wtdn*jet1wtdn*jet2wtdn);
	}
...
int nQCD = max(0, 3 - nTop - nZ - nW - nH - nB);

cout << nTop << nZ << nW << nH << nB << endl;
...
int theseInts[] = {myInts[0], myInts[1], myInts[2]};

std::sort (theseInts, theseInts+3);
std::vector<int> thisPerm;
std::cout << "The possible permutations :\n";
  do {
    std::cout << theseInts[0] << ' ' << theseInts[1] << ' ' << theseInts[2] << '\n';
    thisPerm.clear();
    thisPerm.push_back(theseInts[0]);
    thisPerm.push_back(theseInts[1]);
    thisPerm.push_back(theseInts[2]);
    allPerms.push_back( thisPerm );
  } while ( std::next_permutation(theseInts,theseInts+3) );

  std::cout << "After loop: " << theseInts[0] << ' ' << theseInts[1] << ' ' << theseInts[2] << '\n';
