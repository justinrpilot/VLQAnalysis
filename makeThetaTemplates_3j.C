...
#include "TGraph.h"
//tZ = 1, bW = 2, tH = 3
void makeThetaTemplates_3j(int numTop, int numW, int numZ, int numH, int numB, float br_bw = 0.3333333, float br_tz = 0.3333333, float br_th = 0.333333){
...
c1->SaveAs( TString::Format( "HT3j_%s.pdf", channel.c_str() ) );

gPad->SetLogy(1);

stack->SetMaximum( 1000 * (1+histDATA->GetMaximum()) );
c1_2->Update();
c1->SaveAs( TString::Format( "HT3j_%s_log.pdf", channel.c_str() ) );
