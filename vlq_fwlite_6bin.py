...
parser.add_option('--isData', type='int', action='store', default=0, dest='isData', help='is it Data?')
parser.add_option('--csvSys', type='int', action='store', default=0, dest='csvSys', help='csvSystematic')
...
BEST_SFval = 0.20 #prior SF uncertainty used for systematic templates
...
Z_SFwt = array('f', [0.0])
H_SFwt = array('f', [0.0])
B_misSFwt = array('f', [0.0])
T_misSFwt = array('f', [0.0])
W_misSFwt = array('f', [0.0])
Z_misSFwt = array('f', [0.0])
H_misSFwt = array('f', [0.0])
QCD_SFwt = array('f', [0.0])
csvEventWt = array('f', [-999.])
runNum = array('i', [-999])
lumiNum = array('i', [-999])
eventNum = array('l', [-999])


BESTinputs = np.array(np.zeros((59,1)), dtype=float) 
outTree.Branch('BESTinputs', BESTinputs, 'BESTinputs[59]/D')
...
eventTree.Branch('B_SFwt', B_SFwt, 'B_SFwt/F')
eventTree.Branch('T_SFwt', T_SFwt, 'T_SFwt/F')
eventTree.Branch('W_SFwt', W_SFwt, 'W_SFwt/F')
eventTree.Branch('Z_SFwt', Z_SFwt, 'Z_SFwt/F')
eventTree.Branch('H_SFwt', H_SFwt, 'H_SFwt/F')
eventTree.Branch('B_misSFwt', B_misSFwt, 'B_misSFwt/F')
eventTree.Branch('T_misSFwt', T_misSFwt, 'T_misSFwt/F')
eventTree.Branch('W_misSFwt', W_misSFwt, 'W_misSFwt/F')
eventTree.Branch('Z_misSFwt', Z_misSFwt, 'Z_misSFwt/F')
eventTree.Branch('H_misSFwt', H_misSFwt, 'H_misSFwt/F')
eventTree.Branch('QCD_SFwt', QCD_SFwt, 'QCD_SFwt/F')
eventTree.Branch('csvEventWt', csvEventWt, 'csvEventWt/F')
...
rhoLabel = ("fixedGridRhoAll","","RECO")


#isBHandle = Handle("std::vector<int>")
#isTHandle = Handle("std::vector<int>")
#isWHandle = Handle("std::vector<int>")
#isZHandle = Handle("std::vector<int>")
#isHHandle = Handle("std::vector<int>")
#isBLabel = ("run", "isB", "run")
#isTLabel = ("run", "isT", "run")
#isWLabel = ("run", "isW", "run")
#isZLabel = ("run", "isZ", "run")
#isHLabel = ("run", "isH", "run")
...
if options.isData:
    ...
    jecAK8 = createJEC('Summer16_23Sep2016V4_MC', ['L1FastJet', 'L2Relative', 'L3Absolute'], 'AK8PFchs')
...
for ifile in files:
	...
	for event in events:
		...
		#event.getByLabel(isBLabel, isBHandle)
		#event.getByLabel(isTLabel, isTHandle)
		#event.getByLabel(isWLabel, isWHandle)
		#event.getByLabel(isZLabel, isZHandle)
		#event.getByLabel(isHLabel, isHHandle)

		#isB = isBHandle.product()
		#isT = isTHandle.product()
		#isW = isWHandle.product()
		#isZ = isZHandle.product()
		#isH = isHHandle.product()
		...
		for j in xrange(len(jets)):
			...
			uncorrJet = jets[j].correctedJet(0)
			if not options.isData:
				jecVal = getJEC(jecAK8, uncorrJet, uncorrJet.jetArea(), rho, npv[0])
			else:
				jecVal = getJEC(DataJECs.jecAK8(runnum), uncorrJet, uncorrJet.jetArea(), rho, npv[0])
				

			corr = jecVal *  jets[j].correctedJet(0).pt() / jets[j].pt() 
			corrUp = (jecVal + getJECUnc(jecUncAK8, jets[j],1)) *  jets[j].correctedJet(0).pt() / jets[j].pt()
			corrDn = (jecVal - getJECUnc(jecUncAK8, jets[j],0)) *  jets[j].correctedJet(0).pt() / jets[j].pt()
			...
			#jetLV.SetPtEtaPhiM( jets[j].userFloat('ak8PFJetsPuppiValueMap:pt'), jets[j].userFloat('ak8PFJetsPuppiValueMap:eta'), jets[j].userFloat('ak8PFJetsPuppiValueMap:phi'), jets[j].userFloat('ak8PFJetsPuppiValueMap:mass') )
			#print applyJER(jets[j], products['genJetPt'][j], options.jer)
			jetLV *= corr*applyJER(jets[j], products['genPt'][j], options.jer)
			...
			#if best_bin[0] == 5:
		#		if (isB[j]): B_SFwt[0] += BEST_SFval
		#		else: B_misSFwt[0] += BEST_SFval
		#	elif best_bin[0] == 4:
	#			if (isT[j]): T_SFwt[0] += BEST_SFval
	#			else: T_misSFwt[0] += BEST_SFval
	#		elif best_bin[0] == 3:
	#			if (isH[j]): H_SFwt[0] += BEST_SFval
	#			else: H_misSFwt[0] += BEST_SFval
	#		elif best_bin[0] == 2:
	#			if (isZ[j]): Z_SFwt[0] += BEST_SFval
	#			else: Z_misSFwt[0] += BEST_SFval
	#		elif best_bin[0] == 1:
	#			if (isW[j]): W_SFwt[0] += BEST_SFval
	#			else: W_misSFwt[0] += BEST_SFval
	#		else: QCD_SFwt[0] += BEST_SFval
		...
		if len(savedIndices) >= 3:
			...
			jet1LV *= corrVals[savedIndices[0]]*applyJER(jets[savedIndices[0]], products['genPt'][savedIndices[0]], options.jer)
			jet2LV *= corrVals[savedIndices[1]]*applyJER(jets[savedIndices[1]], products['genPt'][savedIndices[1]], options.jer)
			jet3LV *= corrVals[savedIndices[2]]*applyJER(jets[savedIndices[2]], products['genPt'][savedIndices[2]], options.jer)
			...
			if (len(savedIndices) >= 4):
				...
				jet4LV *= corrVals[savedIndices[3]]*applyJER(jets[savedIndices[3]], products['genPt'][savedIndices[3]], options.jer)
